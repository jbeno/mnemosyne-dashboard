...
export function ReviewPage({ databaseKey }: { databaseKey: string }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="border-l-2 border-primary/45 py-1 pl-5">
        ...
        <p className="mt-2 text-xs tabular-nums text-muted-foreground">{data?.total || 0} total · {items.length} listed · read-only review</p>
      </section>
      {error ? <p className="border-l-2 border-destructive px-4 py-2 text-sm" role="alert">{error}</p> : null}
      <MemoryList empty={loading ? "Loading review queue…" : "This review queue is clear."} items={items} limit={items.length || 1} title="Queue memories" />
